...
#--------------------------------------------------------------------
# procedure‡¥ðRnit_platform_player_locations
...
# ***** Your codes start here: *******
	...
	li $a1, 1 	# $a1 = 1: right
	li $v0, 205
	syscall # set new player direction to right
j pml_exit
# ***** Your codes end here *******
...
# 2. If the player c a boundary, reverse its x_speed: x_speed = 0 - x_speed.
# ******* Your codes start here: ********
...
la $t1, player_size
lw $t1, 0($t1)
blt $t0, $zero, exceeds_left_x #x_loc >= 0
#check if it exceeds the rightmost boundary
add $t0, $t0, $t1  #t0 = x_loc + player_width
la  $t1, width
lw  $t1, 0($t1) #width
bgt $t0, $t1, exceeds_right_x
j cb_check_y_boundary
exceeds_left_x:  #If the player c a boundary, reverse its x_speed: x_speed = 0 - x_speed.
	la $t0, player_locs
	sw $zero, 0($t0)
exceeds:
	la $t0, player_speed
...
type1:
	...
	j plnp_exit
type2:
	la $t0, spring_platform_upward_speed
	lw $t1,  0($t0)
...
platform_Loop:
	...
	beq $s1, $zero, Exit_platform_not_hit#if platform_num == 0, then exit
	la $t2, player_locs
	lw $t2, 4($t2) #player_top_yloc
	lw $t3, 8($s0) #platform_top_yloc
	la $t4, player_size
	lw $t4, 4($t4) #player_height
	blt $t3, $t2, less_than_yloc #If platform_top_y_loc < player_top_y_loc
	add $t2, $t2, $t4 #player_y_buttom_loc 

	blt $t2, $t3, Exit_platform_not_hit #If platform_top_y_loc > player_bottom_y_loc

	la $t2, player_locs
	lw $t2, 0($t2) #player_left_xloc
	#la $t4, player_size
	#sw $t4, 0($t4) #player_width
	#add $t4, $t2, $t4 #player_right_xloc
	la $t4, platform_size
	lw $t3, 4($s0) #platform_xloc
	lw $t4, 0($t4)
	add $t3, $t3, $t4 #platform_right_xloc

	bgt $t2, $t3, Exit_platform_not_hit
	la $t4, player_size
	lw $t4, 0($t4) #player_width
	add $t2, $t2, $t4
	lw $t3, 4($s0) #platform_left_xloc

	blt $t2, $t3, Exit_platform_not_hit
...
check_platform:
	...
	la $t0,  current_platform_address
	lw $t0, 0($t0)
	lw $t2, 0($t0)  #current_id
	lw $t1, 0($s0) #platform_id
	bne $t1, $t2, id_not_equal
	j Exit_platform_hit
id_not_equal:
	la $t0,  current_platform_address
	sw $s0, 0($t0)
...
jr $ra
Exit_platform_not_hit:
lw $s0, 0($sp)
lw $s1, 4($sp)
addi $sp, $sp, 8
li $v0, 0   # Assume the player is not on a platform
jr $ra
# ********** Your codes end here ********
...
destory_platform:
	...
	la $s1 platform_num 
	lw $s1, 0($s1) #platform_num
	addi $s1, $s1, -1
	sll $s1, $s1, 2
	lw $a0, 0($s0)
	li $v0, 208
	syscall #destroy the first platform 
	

update_platform:
	beq $s1,$zero, add_new_platform
	lw $t0, 16($s0)
	sw $t0, 0($s0)
	addi $s0, $s0, 4
	addi $s1, $s1,-1
...
add_new_platform:
	...
	lw $a0, -12($s0) #the last x_loc
	la $a1, width
	lw $a1, 0($a1)
	li $v0, 211
	syscall ##v0 has the new x_loc
	move $a1, $v0
	lw $a0, -16($s0) #the last id
	addi $a0, $a0, 1 #the new id
	lw $t2, -8($s0) #the last y_loc
   	la $t1, platform_height_distance
   	lw $t1, 0($t1)
   	add $t1, $t1, $t2 #new y_loc
	add $a2, $zero, $t1 #new y_loc
	li $v0, 212
	syscall
	move $a3, $v0
	li $v0, 202
	syscall
	#load the info of new platform to "platform"
	sw $a0, 0($s0) 
	sw $a1, 4($s0)
	sw $a2, 8($s0)
	sw $a3, 12($s0)	
